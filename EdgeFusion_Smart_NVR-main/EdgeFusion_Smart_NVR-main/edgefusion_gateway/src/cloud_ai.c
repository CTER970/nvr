/*
 * cloud_ai.c — 云端 AI 识别（图像抓拍 → 上行 → 结果回填）
 *
 * 抓取一帧送云端大模型识别，将结果写入事件中枢；
 * 支持 OpenAI 兼容协议（阿里百炼 / DashScope 等），可热启停。
 */
#include "cloud_ai.h"
#include "log.h"
 ...
 * 适用于阿里百炼/DashScope 的 OpenAI 兼容端点（多模态视觉大模型）。
 * 返回 malloc，失败 NULL */
static char *build_request_body(const char *prompt, const char *model, const char *b64_img)
{
    ...
    /* 部分多模态大模型默认开启思考(reasoning)，会拖慢响应且多耗 token；
