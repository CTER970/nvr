/*
 * alarm.c — 报警输出控制（LED + 蜂鸣器）
 *
 * 根据事件中枢下发的报警状态，驱动板载 LED 与蜂鸣器，
 * 支持初始化、停止和报警参数热更新。
 */
#include "alarm.h"
#include "log.h"
...
int alarm_init(const gateway_conf_t *cfg)
{
    ...
    /* 即使未配置 GPIO 也保持统一线程/订阅流程；gpio_set()会直接忽略负引脚。 */
    g_running = true;
