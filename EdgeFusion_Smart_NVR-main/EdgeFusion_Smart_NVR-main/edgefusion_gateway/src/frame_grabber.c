/*
 * frame_grabber.c — 抓拍帧提取（软解码 → 缩放 → JPEG）
 *
 * 从码流中解码一帧并缩放为 JPEG，供云端 AI 识别与报警快照使用。
 */
