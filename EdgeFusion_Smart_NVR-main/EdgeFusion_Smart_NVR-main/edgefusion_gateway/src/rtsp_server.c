/*
 * rtsp_server.c — RTSP 转发服务
 *
 * 将收到的码流以 RTSP 形式对外转发，供 NVR / 播放器拉流。
 */
...
/* Annex B 由 3/4 字节起始码分隔；当前代码还尝试读取 avcC 配置记录。
 * AVCC packet 的长度前缀尚未单独解析，回调收到的 NALU 不含已识别的分隔字段。 */
typedef void (*nal_cb_t)(const uint8_t *nal, int len, void *user);

/* 返回下一个起始码的位置，并通过 sc_len 返回其3/4字节长度；找不到返回-1。 */
...
/* 根据已收集或缺省的 SPS/PPS 生成 H.264 SDP。 */
...
/* 将一个 H.264 输入包拆成 NALU，并广播为 RTP 包。 */
