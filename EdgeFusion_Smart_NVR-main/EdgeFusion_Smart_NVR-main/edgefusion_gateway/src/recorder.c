/*
 * recorder.c — 录像管理（fragment MP4 切段 + 容量回收）
 *
 * 按关键帧切段写 fMP4，并按容量限额滚动删除最旧录像。
 */
#include "recorder.h"
#include "log.h"
...
/* 将一个 H.264 包重封装到当前 MP4，并在关键帧处按时长切段。 */
int recorder_write_packet(recorder_t *r, AVPacket *pkt, int is_key, double ts)
{
    ...
    /* 写头（首帧，必须在有有效帧时写） */
    if (!r->header_written) {
        /* 当前实现固定输出时基为1/90000，输入时基差异由后续重标定处理。 */
        r->ost->time_base = (AVRational){1, 90000};
        /* fMP4降低对最终trailer的依赖，提高写入中断后的可恢复性；
         * empty_moov先写初始化信息，frag_keyframe按关键帧切fragment，
         * default_base_moof使用相对moof的默认数据偏移。 */
