...
/* 将配置文件中的像素格式字符串（nv21/yv12/nv12/aw_lbc_* 等）映射为 MPP 像素格式枚举。
 * 无法识别时回退为 MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420（即 nv21）。 */
...
/* 将配置文件中的编码器类型字符串（"H.264"/"H.265"）映射为 MPP 负载类型枚举。
 * 无法识别时回退为 PT_H264。 */
...
/* 从 sample_rtsp.conf 加载全部配置：主码流、子码流、延时摄影三路参数，
 * 外加 ISP-VE 联动、回写 YUV、RTSP 网络类型与测试时长。
 * conf_path 为 NULL 时使用代码内默认值。 */
...
/* 配置主码流：VIPPI 采集属性 + VENC 编码属性（H.264/H.265/MJPEG 的码率控制、
 * QP 范围、VBV 缓冲、GOP），并设置 ENCPP 锐化跟随 ISP。 */
...
/* 配置子码流：结构与 configMainStream 相同，额外支持 VIPP 裁剪（crop），
 * 锐化系数按子/主分辨率比例自动折算。 */
...
/* 配置延时摄影码流：复用子码流 VIPP，独立编码通道，结构与子码流一致。 */
...
/* 回写 YUV 线程：在指定起始帧后向编码器请求 WbYuv（缩略图 YUV），
 * 按配置总数循环写文件，用于调试或生成缩略图。 */
...
/* 编码器取流线程：循环 GetStream，将 H.264/H.265 帧写入文件（可选）
 * 并通过 rtsp_sendData 推送到 RTSP 服务，主码流为 I 帧时自动前置 SPS/PPS。
 * 实时统计实际帧率并告警低于目标的情况。 */
