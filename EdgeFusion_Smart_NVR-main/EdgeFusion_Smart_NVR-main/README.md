...
> **声明**：本仓库基于开源项目 [dshanpi/EdgeFusion_Smart_NVR](https://github.com/dshanpi/EdgeFusion_Smart_NVR)（MIT License）进行个人学习、源码审计与改造，遵循其许可证条款；原始版权与许可文本见 [LICENSE](LICENSE)。本仓库相对原项目的修改与发现见文末「本仓库增量」一节。
...
## 本仓库增量（基于原项目的学习、审计与改造）

- **函数注释审计**：为 15 个源文件补充/修正 24 处函数职责注释，不改变代码行为。
- **新增配置样例** [`edgefusion_gateway/config/gateway.conf.example`](edgefusion_gateway/config/gateway.conf.example)：40 个配置键全覆盖，默认值与 `conf.h`/`conf_load` 逐一核对。
- **源码审计发现**（节选，均为学习过程中定位并复核的缺陷，待修复）：
  - `recorder.c`：写帧成功后读取已被 `av_interleaved_write_frame` 置空的 packet 尺寸字段，`bytes_written` 恒 0，Web 端"当前段码率"恒为 0。
  - `recorder.c`：输出时基硬编码 `1/90000` 且未传入真实输入时基，非 90kHz 时基的源会变速播放。
  - `recorder.c`：写头失败后存在非关键帧起段窗口；磁盘满场景下连续写错误计数被非关键帧返回值清零，熔断难以触发。
  - `frame_grabber.c`：`get_jpeg` 在锁外读取共享尺寸字段，存在竞态窗口。
  - `rtsp_server.c`：转发路径时间戳恒 0；`parse_nalus` 对 avcC extradata / AVCC packet 的长度前缀偏移处理不完整。
  - `conf.c`：注释与代码冲突——敏感字段（token 等）实际会被 `conf_save` 写回落盘文件（Web GET 回显已脱敏）。

> 个人学习笔记与完整审计档案不在本仓库范围内；两份学习指南（`PROJECT_LEARNING_GUIDE.md`、`V821_LEARNING_GUIDE.md`）为本地私有材料，不随仓库发布。
