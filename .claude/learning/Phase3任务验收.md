...
| 3.1 | HTTP路由与API地图（含 C4 开场） | 0.75h | **已完成（09-17）** | 四段式 + API 表 |
| 3.2 | MJPEG媒体输出链 | 1h | **已完成（09-17）** | JPEG到浏览器时序 |
| 3.3 | MP4 HTTP Range | 1h | **已完成（09-18）** | 三样例演算 + 分层 |
| 3.4 | socket与客户端线程（含 C3 开场） | 0.75h | **已完成（09-19）** | 两种fd + start≠已监听 |
| 3.5 | 配置保存与热更新边界 | 0.75h | **已完成（09-20）** | 原子写 + 三栏生效表 + 掩码 |
| 3.6 | 配置校验动手（可 AI 代做）与闭卷 | 0.75h | **已完成（09-20，升格为 Phase 3 阶段总验收：C1+三场景+闭卷8题+变式V+90s复述全过；动手=AI 全代做 validate_conf 102 行静态完成）** | 校验 why + 阶段闭卷 |
